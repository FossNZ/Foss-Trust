...
const getInjected = () =>  new Promise((resolve, reject) => {
    window.addEventListener('load', () => {
        if ((window as any).injectedWeb3) { resolve((window as any).injectedWeb3);} 
        else { reject(new Error('injectedWeb3 not found!'));}
    });
});
